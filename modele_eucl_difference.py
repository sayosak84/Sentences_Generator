...
def get_best_word_with_bi_grams(best_words, etiquette_sentence, index):
	...
	best_5_words = list(OrderedDict(sorted(best_words.items(), key=lambda x: x[1], reverse=False)))[:30]
	...
	if (len(words_bi_grams) == 0):
		best_notbigram_words = list(OrderedDict(sorted(best_words.items(), key=lambda x: x[1], reverse=False)))[:10]
		return random.choice(best_notbigram_words)
	else:
		return random.choice(words_bi_grams)


# Recuperer le meilleur mot pour la premiere etiquette
...
# Generer le dictionnaire des distance des mots avec le theme ou la query
...
# Recuperer un dictionnaire des eventuelles meilleurs mots pour la premiere etiquette d'une phrase (filtration)
	...
	for word in dict_theme.keys():
		if word != theme and word != query:
			if theme == query:
				...
				tmp_moyenne = (d_query + d_last_etiquette) - d_theme
...
def display_sentence_from_templates():
	...
	file = open('output/result.txt', 'w', encoding="utf8")
